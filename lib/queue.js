...
const queue = class {
    ...
    async process({
        ...
        _this.max_retry = max_retry
        _this.retry_delay = retry_delay
    ...
    async doWork(work_fn) {
        ...
        return Promise.resolve(work_fn(job.body, job))
            .timeout(
            ...
            .catch(async error => {
                const count_try = await this.dbQry(
                    `SELECT recovered FROM ${this.db_table} WHERE ?`,
                    { id: job.id }
                )
                if (count_try[0].recovered < this.max_retry) {
                    return _this.handleRetry({
                        job_id: job.id,
                        retry_seconds: this.retry_delay,
                        current_try: (count_try[0].recovered ?? 0) + 1
                    })
                }
    ...
    async handleRetry({ job_id, retry_seconds, current_try }) {
        let retry_in = 0
        
        if (typeof retry_seconds === 'number') {
            retry_in = retry_seconds
        } else if (typeof retry_seconds === 'function') {
            retry_in = retry_seconds(current_try)
        }
